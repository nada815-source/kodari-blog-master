  ...
  const copyToClipboard = async (text) => {
    try {
      ...
      let htmlContent = text
        ...
        .replace(/^나름의 해석:.*$/gim, '') 
        .replace(/^### (.*$)/gim, '<p style="font-size: 19pt; font-weight: bold; color: #000; margin-top: 40px; margin-bottom: 10px;"><strong>$1</strong></p>')
        .replace(/^## (.*$)/gim, '<p style="font-size: 19pt; font-weight: bold; color: #000; margin-top: 40px; margin-bottom: 10px;"><strong>$1</strong></p>')
        .replace(/^\* (.*$)/gim, '<li style="font-size: 12pt; margin-left: 20px; margin-bottom: 3px;">$1</li>')
        .replace(/\*\*(.*)\*\*/gim, '<strong>$1</strong>')
        .split('\n').map(line => {
          const trimmed = line.trim();
          if (trimmed === '') return ''; 
          if (trimmed.startsWith('<p style="font-size: 19pt') || trimmed.startsWith('<li')) return trimmed;
          return `<p style="font-size: 12pt; line-height: 1.7; margin-bottom: 15px;">${trimmed}</p>`;
