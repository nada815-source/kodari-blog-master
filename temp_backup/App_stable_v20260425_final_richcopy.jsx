  ...
  const copyToClipboard = async (text) => {
    try {
      ...
      let htmlContent = text
        .replace(/^### (.*$)/gim, '<p style="font-size: 15pt; font-weight: bold; color: #333; margin-top: 20px; margin-bottom: 10px;"><strong>$1</strong></p>')
        .replace(/^## (.*$)/gim, '<p style="font-size: 19pt; font-weight: bold; color: #000; margin-top: 30px; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 10px;"><strong>$1</strong></p>')
        .replace(/^\* (.*$)/gim, '<li style="font-size: 12pt; margin-left: 20px; margin-bottom: 5px;">$1</li>')
        .replace(/\*\*(.*)\*\*/gim, '<strong>$1</strong>')
        .split('\n').map(line => {
          const trimmed = line.trim();
          if (trimmed === '') return '<p><br/></p>';
          if (trimmed.startsWith('<p style="font-size: 15pt') || trimmed.startsWith('<p style="font-size: 19pt') || trimmed.startsWith('<li')) return trimmed;
          return `<p style="font-size: 12pt; line-height: 1.8; margin-bottom: 15px;">${trimmed}</p>`;
        }).join('');
